{
  ...
  launches = pkgs.writeShellScriptBin "launches" ''
    ...
    /run/current-system/sw/bin/hyprctl eval 'hl.dispatch(function()\
      hl.dsp.window.kill({ window = "class:^(${playerClass})$" })\
      hl.dsp.window.kill({ window = "class:^(neo)$" })\
      hl.dsp.window.kill({ window = "class:^(fastfetch)$" })\
      hl.dsp.window.kill({ window = "class:^(btop)$" })\
      hl.dsp.window.kill({ window = "class:^(cava)$" })\
      ...
      hl.dsp.exec_cmd("uwsm app -- localsend_app --hidden")\
    end))'
