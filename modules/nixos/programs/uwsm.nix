{
  ...
  mk_uwsm_desktop_entry = opts: (pkgs.writeTextFile {
    ...
    text = ''
      ...
      Exec=${lib.getExe cfg.package} start -F ${lib.strings.escapeShellArgs opts.extraUWSMArgs} -- ${opts.binPath} ${lib.strings.escapeShellArgs opts.extraArgs}
      Type=Application
    '';
        ...
        lib.types.submodule (
          {...}: {
            options = {
              ...
              extraUWSMArgs = lib.mkOption {
                type = with lib.types; listOf str;
                default = [];
                description = ''
                  Extra command-line arguments pass to UWSM.
                '';
              };
            };
          }
